...
# Система управления видеопотоками и аналитикой v5.5 (оптимизированная с поддержкой StreamRecorder)
...
# ============================================================================
# ОПТИМИЗИРОВАННЫЕ ФУНКЦИИ СОХРАНЕНИЯ КОНФИГУРАЦИЙ
# ============================================================================
...
# ============================================================================
# ФУНКЦИИ ДЛЯ STREAMRECORDER (YUCCA)
# ============================================================================
...
start_stream_recorder() {
    ...
    if docker ps --format "table {{.Names}}" 2>/dev/null | grep -q "recorder"; then
        show_message "Информация" "StreamRecorder уже запущен"
...
show_stream_recorder_status() {
    ...
    if docker ps --format "table {{.Names}}" 2>/dev/null | grep -q "recorder"; then
        ...
        status_info+="🐳 Образ: $container_image\n"
        ...
        status_info+="• Телеметрия: http://${HOST_IP}:${DEFAULT_RECORDER_PORT_TELEMETRY}\n"
        status_info+="• SMTP: ${HOST_IP}:${DEFAULT_RECORDER_PORT_SMTP}\n\n"
...
configure_stream_recorder() {
    while true; do
        local choice
        choice=$(show_menu "КОНФИГУРАЦИЯ STREAMRECORDER" "Настройка параметров StreamRecorder\n\nПорты • Директории • Конфигурация" \
        ...
        case "$choice" in
            "1")
                ...
                if [[ -n "$new_port" ]] && [[ "$new_port" =~ ^[0-9]+$ ]]; then
                    ...
                    init_recorder_config  # Пересоздаем конфигурацию с новыми портами
                else
            ...
            "7")
                if [[ -f "$RECORDER_CONFIG_FILE" ]]; then
                    show_message "Конфигурация docker-compose.yml" "$(cat "$RECORDER_CONFIG_FILE")" 25 90
                else
                    show_message "Ошибка" "Файл конфигурации не найден"
                fi
                ;;
            "8")
                if show_yesno "Сброс конфигурации" "Сбросить конфигурацию StreamRecorder к значениям по умолчанию?\n\nВсе текущие настройки будут потеряны!"; then
...
# ============================================================================
# ОПТИМИЗИРОВАННЫЕ СИСТЕМНЫЕ ФУНКЦИИ
# ============================================================================
...
# ============================================================================
# ОПТИМИЗИРОВАННАЯ ФУНКЦИЯ: ПОЛУЧЕНИЕ РЕЛИЗОВ АГЕНТОВ АНАЛИТИКИ
# ============================================================================
...
# ============================================================================
# ОПТИМИЗИРОВАННЫЕ ФУНКЦИИ ОТОБРАЖЕНИЯ ИНФОРМАЦИИ
# ============================================================================
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        for dep in "${missing[@]}"; do
            if [[ "$dep" == "ffprobe (из пакета ffmpeg)" ]]; then
                ...
                echo -e "  docker-compose: sudo apt-get install docker-compose"
            else
...
collect_logs() {
    ...
    if [[ "$selected_instances" == "all" ]]; then
        ...
        # Добавляем StreamRecorder если запущен
        if docker ps --format "table {{.Names}}" 2>/dev/null | grep -q "recorder"; then
...
# ============================================================================
# РАСШИРЕННАЯ ФУНКЦИЯ ПРОВЕРКИ API (ОПТИМИЗИРОВАННАЯ)
# ============================================================================
...
# ============================================================================
# ФУНКЦИИ ОСТАНОВКИ ВСЕХ АГЕНТОВ (ОПТИМИЗИРОВАННЫЕ)
# ============================================================================
...
show_config_files() {
    ...
    choice=$(show_menu "ФАЙЛЫ КОНФИГУРАЦИИ" "$config_list" \
        ...
        "9" "Конфигурация Yucca (yucca.toml)" \
        "0" "Назад")
    ...
    case "$choice" in
        ...
        "8") [[ -f "$RECORDER_CONFIG_FILE" ]] && show_message "CONFIGURATION STREAMRECORDER" "$(cat "$RECORDER_CONFIG_FILE")" 25 90 || show_message "ОШИБКА" "Файл не найден" ;;
        "9") [[ -f "$YUCCA_CONFIG_FILE" ]] && show_message "КОНФИГУРАЦИЯ YUCCA" "$(cat "$YUCCA_CONFIG_FILE")" 25 90 || show_message "ОШИБКА" "Файл не найден" ;;
        "0") return ;;
    esac
}
